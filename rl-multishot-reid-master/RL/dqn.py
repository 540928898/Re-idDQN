...
#from tensorboard import summary as SummaryWriter
...
def parse_args():
    ...
    parser.add_argument('--num-valid', type=int, default=19529,
                        help='the number of training examples')
    parser.add_argument('--num-id', type=int, default=10, #10
    ...
    parser.add_argument('--num-epoches', type=int, default=40,
    ...
    parser.add_argument('--train-set', type=str, default='image_valid0',
                        help='load model prefix')
    parser.add_argument('--valid-set', type=str, default='image_test0',
                        help='load model prefix')
    ...
    parser.add_argument('--crossvalid', action='store_true', default=True,
...
for e in xrange(args.num_epoches):
    ...
    for batch in xrange(iterations):
        ...
        Qvalue_softmax = mx.nd.SoftmaxActivation(mx.nd.array(Qvalue, ctx=mx.cpu()) / epsilon / 5).asnumpy()
        ...
        for b in xrange(args.batch_size):
            ...
            agent.update([mx.nd.array(grad, ctx=mx.cpu()), mx.nd.zeros(agent.Q.get_outputs()[1].shape, ctx=mx.cpu())])
    ...
    fts = [[(0,0) for _2 in xrange(N)] for _1 in xrange(N)]
    while valid_cnt*valid_cnt < N*N:
        if args.verbose:
