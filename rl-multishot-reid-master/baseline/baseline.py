...
def get_imRecordIter(name, input_shape, batch_size, kv, shuffle=False, aug=False, even_iter=False):
    ...
    if even_iter:
        ...
        dataiter =mx.image.ImageIter(batch_size=4, data_shape=(3, 224, 112),
                              path_imglist="%s/%s.lst" % (args.data_dir, name),
                              path_root="/opt/iLIDS-VID/i-LIDS-VID" )
...
train, val = get_iterators(
    ...
    train=args.train_file, test=args.test_file, kv=kv, gpus=1)
